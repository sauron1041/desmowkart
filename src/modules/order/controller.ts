...
export class SkillController {
    ...
    constructor() {
        this.categoryService = new OrderService();
    }
    public create = async (req: Request, res: Response) => {
        const model: User = req.body as any as User;
        model.orderDetails
        try {
            // public create = async (model: Partial<Skill & { orderDetail?: OrderDetailModel[] }>) => {
    ...
    public countTotalPurchased = async (req: Request, res: Response) => {
        try {
            const customerId: number = req.query.customerId as any;
            const serviceId: number = req.query.serviceId as any;
            const result = await this.categoryService.countTotalPurchased(customerId, serviceId);
            if (result instanceof HttpException) {
                return sendResponse(res, result.status, result.message);
            }
            return sendResponse(res, 200, message.FIND_SUCCESS, result);
