...
import { ServiceRequestHistoryService } from "./service";
...
export class SkillController {
    private categoryService: ServiceRequestHistoryService;
    constructor() {
        this.categoryService = new ServiceRequestHistoryService();
    }
    ...
    public findAllServiceRequestHistoryByServiceId = async (req: Request, res: Response) => {
        try {
            ...
            const result = await this.categoryService.findAllServiceRequestHistoryByServiceRequestId(serviceId);
            if (result instanceof HttpException) {
                return sendResponse(res, result.status, result.message);
            }
            return sendResponse(res, 200, message.FIND_ALL_SUCCESS, result);
