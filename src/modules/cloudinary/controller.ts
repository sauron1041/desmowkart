...
import { CloudDinaryService } from "./service";
...
export class CloudDinaryController {
    private categoryService: CloudDinaryService;
    constructor() {
        this.categoryService = new CloudDinaryService();
    }
    ...
    // public findAllServiceRequestHistoryServiceId = async (req: Request, res: Response) => {
    //     try {
    //         const model: User = req.query as any as User;
    //         const search: ISearchAndPagination = req.query as any as ISearchAndPagination;
    //         const result = await this.categoryService.findAllServiceRequestHistoryServiceId(model, search);
    //         if (result instanceof HttpException) {
    //             return sendResponse(res, result.status, result.message);
    //         }
    //         return sendResponse(res, 200, message.FIND_ALL_SUCCESS, result);
    ...
    public findAllServiceRequestHistoryByServiceId = async (req: Request, res: Response) => {
        try {
            const serviceId: number = req.params.serviceId as any;
            const result = await this.categoryService.findAllServiceRequestHistoryByServiceId(serviceId);
            if (result instanceof HttpException) {
                return sendResponse(res, result.status, result.message);
            }
            return sendResponse(res, 200, message.FIND_ALL_SUCCESS, result);
    ...
    public uploadImageCloud = async (req: Request, res: Response, next: NextFunction) => {
        const file = req.file;
        try {
            const result = await this.categoryService.uploadImageCloud(file as any);
            if (result instanceof Error)
                return sendResponse(res, (result as any).status, result.message);
            return sendResponse(res, 200, message.UPLOAD_SUCCESS, result);
        } catch (error) {
            next(error);
        }
    }
