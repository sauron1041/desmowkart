import { CloudDinaryController } from "./controller";
import { IRoute } from "@core/interfaces";
import { Router } from "express";
import multer from "multer";
import { AuthMiddleware } from "@core/middleware";

export class CloudDinaryRoute implements IRoute {
    public path = '/cloudinary';
    public router = Router();
    public upload = multer({ storage: multer.memoryStorage() });

    public skillController = new CloudDinaryController();

    constructor() {
        this.initializeRoutes();
    }
    private initializeRoutes() {
        this.router.post(this.path + '/upload', this.upload.single('file'), this.skillController.uploadImageCloud)
        ...
        this.router.get(this.path + '/find-all-service-request-history-by-service-id/:serviceId', AuthMiddleware.authorization, this.skillController.findAllServiceRequestHistoryByServiceId);
