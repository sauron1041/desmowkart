...
import { ServiceRequestImageService } from "./service";
...
export class ServiceRequestImageController {
    ...
    constructor() {
        this.categoryService = new ServiceRequestImageService();
    }
    public create = async (req: Request, res: Response) => {
        const model: User = req.body as any as User;
        model.employeeId = req.id as any;
    ...
    public findAllServiceRequestImageByServiceId = async (req: Request, res: Response) => {
        try {
            const serviceId: number = req.params.serviceId as any;
            const result = await this.categoryService.findAllServiceRequestImageByServiceId(serviceId);
            if (result instanceof HttpException) {
                return sendResponse(res, result.status, result.message);
            }
            return sendResponse(res, 200, message.FIND_ALL_SUCCESS, result);
